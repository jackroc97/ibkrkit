...
async def wait_for_ibkr_ready(host="127.0.0.1", port=4004, client_id: int = 99, retries=30, delay=5):
    ...
    for attempt in range(1, retries + 1):
        try:
            log(LogTag.INFO, f"Attempt {attempt}: Connecting to IBKR at {host}:{port}...")
            await ib.connectAsync(host, port, clientId=client_id)
            if ib.isConnected():
                log(LogTag.INFO, "IBKR connection established and ready.")
                ib.disconnect()
                return True
        except Exception as e:
            log(LogTag.WARN, f"Connection failed ({e}). Retrying in {delay}s...")
        await asyncio.sleep(delay)

    log(LogTag.WARN, "Failed to connect to IBKR after multiple attempts.")
    return False
